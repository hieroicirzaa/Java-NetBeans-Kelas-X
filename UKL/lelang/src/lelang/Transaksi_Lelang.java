    ...
    public Transaksi_Lelang(Masyarakat Masyarakat, Petugas Petugas, Barang Barang) {
        ...
        do {
            ...
            if (confirm == 1) {
                ...
                System.out.println("Barang Yang Akan Di Lelang Adalah \t: " + Barang.getBarang(IdBarang - 1));
                if (!Barang.getStatus(IdBarang - 1)) {
                    System.out.println("Anda Ingin Mengubah Item : " + Barang.getBarang(IdBarang - 1) + "\nMenjadi Bisa Di Tawar ? [Y|N]");
                    System.out.print("Pilihan Anda \t: ");
                    String Option = input.next();
                    if (Option.equalsIgnoreCase("Y")) {
                        Barang.setStatus(IdBarang - 1, true);
                        System.out.println("\nItem : " + Barang.getBarang(IdBarang - 1) + " Kondisi Barang " + Barang.getStatus(IdBarang - 1));
                        if (Barang.getStatus(IdBarang - 1)) {
                            System.out.println("Barang Dapat Di Lelang");
                            System.out.println("Maksimal 3x Bid/Penawaran");
                            int max = 0;
                            ...
                            do {
                                ...
                                if (id < 1 || id > 3) {
                                    ...
                                    System.out.println("Masukan Bid Anda.(Minimal Bid " + Barang.getHarga(IdBarang - 1) + ")");
